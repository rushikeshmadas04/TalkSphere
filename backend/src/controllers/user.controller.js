  ...
  try {
    ...
    const recommendedUsers = await User.find({
      $and:[
        ...
        {_id: {$nin: currentUser.friends}},//Fixed: was $id, should be _id
        {isOnboarded: true}
      ]
    })
...
export async function acceptFriendRequest(req,res){
  try {
    ...
    //$addToSet: adds elements to a array only if they do not already exist.
...
export async function getFriendRequests(req,res){
  try {
    const incomingRequs = await FriendRequest.find({
    ...
    res.status(200).json({incomingRequs,acceptedReqs});
  } catch (error) {
